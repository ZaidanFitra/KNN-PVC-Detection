...
#define offsetECG_B 4.21

volatile uint32_t bufferECG[bufferECG_size][4];
...
String formatUnixMillis(uint64_t unixMillis, int timezoneOffsetHours) {
  ...
  struct tm *tmInfo = gmtime(&seconds);
  char buffer[30];
...
// Fungsi menulis data ke SD card
...
// Fungsi untuk buffering data sebelum ditulis ke SD card
...
void setup() {
  ...
  sdc.begin(SD_SCK, SD_MISO, SD_MOSI, SD_SS);
  ...
  // Pengecekan jumlah file pada SD card
  ...
  // Inisialisasi BLE
  BLEDevice::init("ETA-1_HeartSense");
...
void loop() {
  if (deviceConnected && sendData == 1) {
    // Kedip setiap 4 detik saat recording
  ...
  if(bufferECG_p2 != bufferECG_p){
    ...
    if(sendData==1){
      ...
      // Pembungkusan data untuk BLE
      int offset = sampleCount * SAMPLE_SIZE;
      memcpy(&sendBuffer[offset], &sample.timestamp, 8);   
      memcpy(&sendBuffer[offset + 8], &sample.ecgData, 4);  

      sampleCount++;

      // Pengiriman data via BLE
      ...
      // Pembungkusan dan penulisan data ke SD card
